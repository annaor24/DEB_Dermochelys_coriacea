% modeling food and temp

clear all 
%close all
datelog = date;
scenario = 'Temp'; % options: Food, Temp
% save ('results_fT.mat', 'datelog') % comment out after the first run

red = [1 0 0] ; %  30   % -50%
orange = [1 0.4 0]; % 28  %  -30%
yello = [1 1 0]; % 26  %   -20%
green = [0 0.6 0]; % 24  % - 10%
blue = [0 0 1]; % 22  % 0
dblue= [0 0 0.4]; % 20  %  +20%
purple = [0.6 0 0.6]; % 18 % +50%
black = [0 0 0 ]; % 16  % +100%
...
colors = [red; orange; yello; green; blue; dblue; purple; black; red; orange; yello; green; blue; dblue; purple; black];
 ...
 %% 1 - initialize time, parameters, etc
  simu = init ;
     K1 = 174.25; f = 0.8987; T=21.13; % ANNA CHANGE FROM f = 0.81 and T = 21.8
     Xinit = f .* simu.par(22) ./ (1-f);  % f = x/ (1+x) = X/K / (1 + X/K)

%% simulate conditions

limit = 8; % this will simulate current conditions +7points lower and 7points higher than current

switch scenario
    case 'Food'                        
%         markers = ['o', '.', 'x','.', 's','*'];
     %  food (ff)
   mdfyX = [linspace(0.25,1,limit) linspace(1,3,limit)] ; % to ensure f is simulated
   ...
   mdfyX= fliplr(mdfyX); % better for plotting
        ...
        for ii = 1: length(mdfyX)
          simu.Xinit =  Xinit * mdfyX(ii);
            ...
            i_sp = find(and((simu.tEVHR(:,5) == 0),( simu.tEVHR(:,4)>=simu.par(14)) ) ) ;
...
%      ylabel('eggs per clutch (#)')
    ...
    case 'Temp'
        ...
        % different temperature
         Ts = [linspace(14,T,limit) linspace(T,30,limit)] ; % to ensure T is simulated
       ...
       fprintf ('======================================================= \n')
        ...
        for p = 1: length(Ts)            
                fprintf('%2.2f & %2.2f & %4.1f & %1.3f  & %3.2f& %3.2f & %3.2f & %i \\\\ \n',...
                   Ts(p), Ap_T(p)/365, Lp_T(p), Wp_T(p)/1000, Li_T(p), Wi_T(p)/1000, round(Ri_T(p)), round(cumF_T(p)))           
        end
        fprintf ('======================================================= \n')
