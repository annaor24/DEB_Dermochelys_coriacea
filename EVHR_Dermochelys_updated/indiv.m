function tEVHR = indiv(simu)
 %---------------------------------------------------------------
  ...
  % calls : fluxes.m
  %
  % 2024-Sep-20 - Nina Marn (modified); 2013/03/15 - Laure Pecquerie(created basic scripts)
  %--------------------------------------------------------------
  ...
  while tc < simu.tm
      i = i + simu.delR/365; % nesting every delR years (delR set in init.m)
...
%       options = odeset(options, 'Events',@eventsfun); % add an events function  -> does not work!
...
EH_p      = simu.par.E_Hp; % maturity at puberty
